// Newline-delimited JSON framing codec — desktop transport.
// Mirrors services/fs-sync/src/ipc/framing.ts byte-for-byte; see
// tasks.md 3.2 for the copy-vs-share decision (shared package is a
// follow-up). Any change here MUST be mirrored on the service side
// until the shared module lands.
//
// One frame per line. Frames may be arbitrarily split across chunk
// events, and a single chunk may carry multiple frames. Lines that
// exceed `maxFrameBytes` are rejected with `FrameTooLargeError`;
// lines that fail `JSON.parse` produce a `FrameParseError`. The codec
// does NOT validate Request/Response/Event shape — the caller owns
// that.
