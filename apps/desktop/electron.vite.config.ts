...
// D6: Build-time OAuth client credentials are inlined via esbuild `define`.
//
// Vite's `loadEnv()` reads `.env`, `.env.local`, `.env.<mode>`, etc. from
// the apps/desktop directory and returns them as a plain object — it does
// NOT mutate `process.env`. We then layer `process.env` over it so CI's
// GitHub Actions secrets (passed as real env vars, not files) and dev's
// `.env.local` both work without per-environment branching.
//
// The third argument `""` to loadEnv disables the default `VITE_` prefix
// filter — internal main-process env vars do not use that prefix.
//
// In a non-dev build with both sources empty, fail fast so a stale GCP
// rotation never produces a packaged binary that throws at first click.
export default defineConfig(({ mode }) => {
  const env = loadEnv(mode, __dirname, "");
  const clientId = env["FT5_GOOGLE_OAUTH_CLIENT_ID"]
    ?? process.env["FT5_GOOGLE_OAUTH_CLIENT_ID"]
    ?? "";
  const clientSecret = env["FT5_GOOGLE_OAUTH_CLIENT_SECRET"]
    ?? process.env["FT5_GOOGLE_OAUTH_CLIENT_SECRET"]
    ?? "";

  if (mode !== "development") {
    if (!clientId) {
      throw new Error(
        "[ft5] FT5_GOOGLE_OAUTH_CLIENT_ID must be set for non-development builds. "
        + "In CI, pass it via GitHub Actions secrets. Locally, set it in apps/desktop/.env.local.",
      );
    }
    if (!clientSecret) {
      throw new Error(
        "[ft5] FT5_GOOGLE_OAUTH_CLIENT_SECRET must be set for non-development builds. "
        + "In CI, pass it via GitHub Actions secrets. Locally, set it in apps/desktop/.env.local.",
      );
    }
  }
  ...
  return {
    main: {
      define: {
        __FT5_GOOGLE_OAUTH_CLIENT_ID__: JSON.stringify(clientId),
        __FT5_GOOGLE_OAUTH_CLIENT_SECRET__: JSON.stringify(clientSecret),
      },
      plugins: [
