...
### 5.B Engine: no-creds construction path (prerequisite for 5.A.7+)

Discovered during 5.A prep: `factory.create` requires `StoredCredentials`, but `authenticate()` is the method you call WHEN you don't have creds yet. Every strategy factory reads creds via `readCredsFromStored(credentials)` which throws on empty fields. The `doAuthenticateImpl` methods themselves don't touch the pre-existing creds — only the operational calls (upload, list, etc.) do. So the minimal fix: let `StoredCredentials | null` flow through construction; operational calls throw "requires authentication" if null; `authenticate()` works unchanged.

- [ ] 5.B.1 RED: `packages/fs-datasource-engine/src/factory.test.ts` — extend with tests that `factory.createForAuth(providerId, datasourceId, ctx)` returns a `DatasourceClient<P>` on which `authenticate()` succeeds (all three providers), but `listDirectory` / `uploadFile` / etc throw a clear `DatasourceError` with `tag: "auth-revoked"` (or equivalent — pick the tag that matches the existing "no creds" error). `ProviderFactoryFn<P>` signature accepts `StoredCredentials | null`.
- [ ] 5.B.2 GREEN: edit `packages/fs-datasource-engine/src/factory.ts` — `ProviderFactoryFn<P>` accepts `StoredCredentials | null`; add `ClientFactory.createForAuth(providerId, datasourceId, ctx)` that calls the registered factory fn with `null`.
- [ ] 5.B.3 RED: `packages/fs-datasource-engine/src/strategies/s3-client.test.ts` — construct via `createS3Client(datasourceId, null, ctx)`; `authenticate()` returns the credentials-form intent; `listDirectory` throws auth-revoked.
- [ ] 5.B.4 GREEN: edit `packages/fs-datasource-engine/src/strategies/s3-client.ts` — factory fn accepts `StoredCredentials | null`, passes `null` through. `S3Client` constructor stores `creds: S3CredsMeta | null`. Operational methods throw auth-revoked on null creds.
- [ ] 5.B.5 RED: `packages/fs-datasource-engine/src/strategies/googledrive-client.test.ts` — same shape.
- [ ] 5.B.6 GREEN: edit `packages/fs-datasource-engine/src/strategies/googledrive-client.ts` — same shape.
- [ ] 5.B.7 RED: `packages/fs-datasource-engine/src/strategies/onedrive-client.test.ts` — same shape.
- [ ] 5.B.8 GREEN: edit `packages/fs-datasource-engine/src/strategies/onedrive-client.ts` — same shape.
- [ ] 5.B.9 Request code review on the engine change — the factory signature touches every strategy; reviewer confirms operational calls actually do throw on null creds and `authenticate()` is the only method that doesn't need them

### 5.A (resumed)

- [ ] 5.A.7 RED: `services/fs-sync/src/commands/__tests__/authenticate-start.test.ts` — handler calls `factory.createForAuth(type, datasourceId, { bus, credentialStore })`, invokes `client.authenticate()`, stashes the returned intent by correlation id, returns the serialized descriptor (oauth → `{ kind, authorizeUrl }`, form → `{ kind, schema }`)
