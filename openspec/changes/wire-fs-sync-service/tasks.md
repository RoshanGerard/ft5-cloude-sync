...
- [x] 6.5 RED: `preload/__tests__/window-api.types.test-d.ts` — extend `apps/desktop/src/preload/window-api.d.ts` with the typed `sync` branch; type assertion that `window.api.sync.listJobs` has the expected return type — commit <sha-pending>. Note: type assertions pass vacuously pre-GREEN because vitest typecheck resolves Window["api"]["sync"] as `any` when window-api.d.ts lacks the sync branch. The REAL enforcement is that `pnpm typecheck` would fail for any renderer code attempting to use `window.api.sync.*` before 6.6.
