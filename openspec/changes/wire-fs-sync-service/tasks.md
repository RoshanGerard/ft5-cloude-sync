...
- [ ] 5.A.7 RED: `services/fs-sync/src/commands/authenticate-start.test.ts` — handler returns `{ ok: false, error: { tag: "not-implemented", message: "authenticate flow pending follow-up change (see design.md Decision 11)" } }` regardless of params; does NOT touch the correlation store; does NOT invoke any engine/factory/client. Contract error-union extension: `{ tag: "not-implemented"; message: string }` added to `AuthenticateStartCommand.error`.
- [ ] 5.A.8 GREEN: implement `services/fs-sync/src/commands/authenticate-start.ts` as stub; wire into `buildCommandHandlers` (matches existing handler.ts pattern); extend the wire contract error union for the tag. Header comment in the handler module points at Decision 11 and the follow-up change.
- [ ] 5.A.9 RED: `services/fs-sync/src/commands/authenticate-complete.test.ts` — handler returns the same `not-implemented` error shape; does NOT look up correlation ids; does NOT invoke any closure. Contract error-union extension in `AuthenticateCompleteCommand.error` to match.
- [ ] 5.A.10 GREEN: implement `services/fs-sync/src/commands/authenticate-complete.ts` as stub; wire into `buildCommandHandlers`; extend the wire contract error union; same "pending follow-up" header comment.
- [ ] 5.A.11 Update `services/fs-sync/src/observability/logger.ts` — redact both `sync:authenticate-start` params and `sync:authenticate-complete` params/result as `[redacted]`; update `logger.test.ts` accordingly. The stubbed handlers never see real tokens, but the redaction rule is defensive and must land with the channel names so the follow-up change doesn't accidentally ship before it.
