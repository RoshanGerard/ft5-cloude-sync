...
- [x] 10.8 Request code review for Phase 10. Per-task two-stage reviews (spec-compliance + code-quality subagents) ran against each Phase 10 commit (3cb8606 event-bridge, 5c0daa4 preload onEvent, b3ddfeb renderer useDatasourceEvents). All three approved; one Important observation surfaced — the `apps/desktop/src/preload/window-api.d.ts` and `apps/desktop/src/renderer/src/types/window-api.d.ts` ambient `Window.api` declarations are independent and can drift (the renderer header acknowledges this, but nothing enforces structural equality). Reviewer's own assessment: "not blocking this phase; worth filing a follow-up." Captured as a known debt here rather than a new OpenSpec change, because (a) it predates this change and (b) the fix (shared canonical interface in `@ft5/ipc-contracts`) is a cross-cutting refactor better scoped to its own proposal.
...
- [x] 11.5 Manually exercise the feature flag path with `DATASOURCE_ENGINE_LIVE=1 pnpm -C apps/desktop dev`: add a mock S3 datasource (access-key form), trigger a list, observe a `status-changed` or `file-created` event in the renderer DevTools, and confirm no crash / no regressions in the dashboard. Verified 2026-04-21: added an S3 datasource with bogus creds, clicked sync-now, observed `EVT {event: 'status-changed', datasourceType: 'amazon-s3', datasourceId: 'ds-amazon-s3-…', ts: …, payload: {…}}` in DevTools via `window.api.datasources.onEvent(cb)`, card transitioned to Error with reason `auth-revoked: [auth-revoked] …` (matching the exact format action.ts produces on the live path). Full round-trip engine → bus → event-bridge → preload → renderer callback confirmed live end-to-end. No crash, no regressions in the dashboard. Pre-existing Electron dev-mode CSP warning unrelated. Environment note: a `better-sqlite3` NODE_MODULE_VERSION mismatch on the worktree required `npx @electron/rebuild -f -w better-sqlite3` to rebuild against the current Electron ABI — not introduced by this change; the archived `allow-native-build-scripts` covers the standard recovery path.
...
- [x] 12.2 Event replay: confirm the "late subscriber sees only future events" policy and add a renderer-side test verifying a terminal event is still delivered if subscribe happens after upload-start but before upload-completion.
