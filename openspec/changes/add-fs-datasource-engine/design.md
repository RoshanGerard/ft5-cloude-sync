...
**Phase 11.1 adds two package dependencies to `apps/desktop` (dev-only):**

- **`@aws-sdk/client-s3` (dev)** and **`aws-sdk-client-mock` (dev)** — needed by the engine smoke integration test at `apps/desktop/src/main/__tests__/engine-smoke.test.ts`. The test drives an end-to-end upload round-trip through the real `S3Client` strategy with the AWS SDK command-dispatch layer intercepted by `mockClient(AwsS3Client)`. The same two specifiers already ship as deps of `@ft5/fs-datasource-engine` (runtime + dev respectively); declaring them here gives pnpm a resolvable path from `apps/desktop`'s own `node_modules`. Both are confined to `devDependencies`, so neither ships in the Electron bundle. The smoke test lives under `main/__tests__/` rather than the spec-prescribed `ipc/__tests__/` path because the Phase 9.12 guardrail (`no-provider-sdk-imports.test.ts`) forbids direct SDK imports anywhere in the `ipc/` subtree — relocating one directory up preserves both invariants.
...
- **RESOLVED (Phase 9 — scoping) — `files/*` IPC handlers are deferred to `ui-file-explorer`.** This change was drafted assuming `ui-file-explorer`'s handler surface (`files:list / files:stat / files:search / files:rename / files:remove / files:download`) existed on the trunk when Phase 9 ran. It does not — `ui-file-explorer` is still in-progress on a separate worktree, and the engine lacks `rename` / `download` primitives regardless. Phase 9 now covers ONLY the `datasources/*` handler surface. The engine-backed rewrite of `files/*` lands as part of `ui-file-explorer` (or a small follow-up change after it merges). No contract conflict — the engine is called BY those handlers; it is not a replacement for their `ipc-contracts` types.
- **RESOLVED (Phase 9 — scoping) — Phase 9 owns the first real main-process DB open.** Phase 4 landed `SqliteCredentialStore` but no caller ever opened a DB; the store lived only in its own tests. Phase 9 introduces `openDatabase(app.getPath("userData")/ft5.db)` + a migration runner (`apps/desktop/src/main/db/`) that owns migrations `0001_datasource_credentials` (re-issued from the store's inline migration, for consolidation) and `0002_datasources` (the new datasource-registry table). The shared DB handle is passed to `SqliteCredentialStore` and to the new `DatasourceRegistry`. Stub-then-replace was rejected as wasted work. The store's inline `CREATE TABLE IF NOT EXISTS` has been removed so the runner is the single source of truth for schema; a future `ALTER TABLE` migration cannot be silently shadowed by defense-in-depth table creation.
- **RESOLVED (Phase 9 — scoping) — `datasources/*` management handlers get a persistent DB-backed registry.** The existing `apps/desktop/src/main/ipc/datasources/store.ts` is an in-memory fixture. Phase 9 replaces it with `DatasourceRegistry` (backed by the new `datasources` SQLite table) so `list` / `add` / `remove` / `action` survive restart and compose cleanly with `SqliteCredentialStore`. `action: "pause" | "resume"` flips a persisted `paused` flag; `action: "sync-now"` calls `client.status()` and updates `itemCount` / `lastSyncAt` on the registry row — a proper sync queue (background jobs, retry, backoff) is a follow-up change, not part of this engine change. `upload` is the only file-operation handler rewired in Phase 9; it resolves the provider via the registry and calls `client.uploadFile(parent, file)` with a path-form `Target`.
...
- **RESOLVED (Phase 12 — scoping) — Per-datasource mutation queues are OWNED BY `fs-sync-service`, not this engine.** The engine is a shared module providing the common provider interface; `fs-sync-service` is the background job queue that sequences mutations (uploads, deletes, renames) across datasources. Landing a per-datasource queue inside `BaseDatasourceClient` would duplicate responsibility and give two serialization layers to reason about when races appear. The engine therefore exposes its operations as directly-awaited async methods; callers that need ordering guarantees route through the sync-service queue (change `add-fs-sync-service`, proposed but not yet applied — depends on this engine merging to master first). If future integration testing reveals race scenarios that `fs-sync-service` cannot address (e.g., strategy-internal re-entrancy), revisit as a separate change.
- **RESOLVED (Phase 12) — Event replay policy: late subscribers see only future events; terminal events still flush through the coalescer.** The `EventBus` is stateless across subscribers — a handler registered AFTER an emit does not receive it retroactively. Streaming is ephemeral by design (progress is volatile), and terminal events (`file-created`, `upload-failed`, `deleted`, `token-refreshed`, `token-expired`) are authoritative: if a subscriber registers between the last `uploading` emit and the `file-created` emit, it still sees `file-created` (because that emit has not yet occurred) but it does NOT see the prior streaming ticks. A renderer-side test in Phase 12.2 verifies this: subscribing between `uploading` and `file-created` delivers the terminal. No event buffer, no replay log — consumers needing state-of-world rehydrate via query (`list`, `status`) after subscribing.
- **Upload cancellation — follow-up change `add-fs-engine-cancellation`.** The public interface does not expose `cancelUpload(transactionId)` yet. A stub proposal has been filed at `openspec/changes/add-fs-engine-cancellation/proposal.md` capturing scope (add `cancelUpload` to `DatasourceClient`, wire each provider SDK's cancellation primitive, define cancellation events / payload shape) so the decision is durable and not lost between changes. Explicitly NOT in scope for this change.
- **RESOLVED (Phase 12) — `authentication-failed` event payload carries the full serialized `DatasourceError`.** `PayloadMap[T]["authentication-failed"]` is the serialized DatasourceError shape (`{ tag, datasourceType, datasourceId, retryable, retryAfterMs?, raw?, message }`) — not a bare reason string. Rationale: (a) consumers (audit log, telemetry, toast UI) need `retryable` / `retryAfterMs` to present recovery affordances correctly; (b) the `raw` payload preserves provider-specific diagnostic context that a reason string would discard; (c) the Flavour B payload typing already supports structured payloads per provider, so there is no typing cost. Structured-clone across IPC drops the class identity (subscribers receive a plain-object shape, not a `DatasourceError` instance), which is documented on the renderer hook in Phase 10.7 so consumers reconstruct via field access rather than `instanceof`.

## Known debt for follow-ups

Captured during the Phase 11.6 final code review. None of these block archive; each is a forward-looking item the next change should pick up.

- **Shared canonical `Window.api` interface.** `apps/desktop/src/preload/window-api.d.ts` and `apps/desktop/src/renderer/src/types/window-api.d.ts` are independent ambient declarations that happen to be structurally equivalent today but can drift silently. Recommended scope: a canonical `WindowApi` interface in `@ft5/ipc-contracts`, consumed by both ambient files. Alternative: a tsc structural-equality test between them. Flagged in two reviews (Phase 10.6/10.7 code review and the Phase 11.6 final review).
- **Strategy lifecycle / `dispose()` ownership.** `BaseDatasourceClient` exposes a `dispose()` that strategies with bus subscriptions (OneDrive + GoogleDrive, for their path↔handle cache invalidation) rely on. No caller in this change actually caches clients per `(providerId, datasourceId)` — the `ClientFactory` documents this as a deferred concern. The moment a caching handler lands (most likely in `add-fs-sync-service` or the `files/*` rewire follow-up), it MUST call `dispose()` on eviction or the bus subscription leaks. Either fold this into the caching caller or ship a small `ClientCache` helper at the engine package level.
- **Error taxonomy: 9-tag (`quota-exceeded`) or `providerMetadata.reason` convention.** The current 8 tags flatten storage-quota exhaustion into `provider-error`. OneDrive's `normalizeError` explicitly documents this (Graph `quotaLimitReached` → `provider-error` with the code preserved in `raw`). Worth either (a) adding a ninth tag with `retryable: false`, or (b) defining a reason-string convention on `raw` that every strategy populates. Expand the taxonomy carefully — consumer-branch count is the downside.
- **S3 `normalizeError` skips `auth-expired` on purpose.** The strategy maps AWS `ExpiredToken` / `AccessDenied` / 403 to `auth-revoked` because static IAM credentials cannot be refreshed — triggering the base's single-flight refresh would be wasted work. Correct behaviour, documented in the class header, but a cross-reference from `s3-client.ts`'s `normalizeErrorImpl` back to Decision 9 would save a future reader the detective work.
- **Stalled-upload watchdog.** The Risks section of this design document describes a 30-second no-progress watchdog emitting `status-changed { status: "stalled" }` — that code was deferred. Worth filing a one-line tracking entry at proposal time for the next change that touches uploads.
- **Per-datasource mutation serialization.** Intentionally owned by `add-fs-sync-service`. Already captured in Open Questions above.
- **Upload cancellation.** Stub proposal at `openspec/changes/add-fs-engine-cancellation/proposal.md`. Already captured in Open Questions above.
