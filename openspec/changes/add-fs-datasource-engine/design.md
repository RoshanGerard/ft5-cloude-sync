...
- **RESOLVED (Phase 9 — scoping) — Phase 9 owns the first real main-process DB open.** Phase 4 landed `SqliteCredentialStore` but no caller ever opened a DB; the store lived only in its own tests. Phase 9 introduces `openDatabase(app.getPath("userData")/ft5.db)` + a migration runner (`apps/desktop/src/main/db/`) that owns migrations `0001_datasource_credentials` (re-issued from the store's inline migration, for consolidation) and `0002_datasources` (the new datasource-registry table). The shared DB handle is passed to `SqliteCredentialStore` and to the new `DatasourceRegistry`. Stub-then-replace was rejected as wasted work.
