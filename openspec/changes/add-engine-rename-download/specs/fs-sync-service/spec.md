...
The `files:download` handler SHALL emit consumer-domain events on the service's IPC event channel — these events are NOT generated by the engine bus (the engine has no `downloading` taxonomy per the engine spec). The wire shapes:
...
The handler invokes the engine's `onProgress` callback hook to drive progress emissions: each `(loaded, total)` callback maps to one `downloading` event after coalescing. Terminal events emit exactly once per download.
