...
None outstanding (resolved via brainstorming + advisor checkpoint 1). Implementation detail to finalize at apply: the exact contract-suite assertion shape — white-box `cache.has(path) === false` after the op (mirrors the existing `hasPathHandleCache` upload assertion) vs behavioral "re-address re-resolves (provider spy)". Lean white-box for consistency with the existing assertion.
