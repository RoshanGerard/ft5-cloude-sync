...
When the service-config-missing error tag arrives via `auth-failed { tag: "service-config-missing", path, providerId }`, the form SHALL render an inline message reading "Service configuration missing. Add OAuth credentials to `<path>` (rendered as inline code). See README §Provider OAuth registration." A Retry control SHALL be available — though it will surface the same error until the user fixes the file.
