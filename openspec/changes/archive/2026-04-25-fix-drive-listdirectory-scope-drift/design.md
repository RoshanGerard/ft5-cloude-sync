...
**Bus event vs thrown error.** The structured `raw.kind` discriminator lives on the THROWN `DatasourceError`, which is what direct callers (the IPC handler that calls `engine.status()`) receive. Bus subscribers receive a `status-changed` event whose payload is `{ status: "error", error: <tag> }` — this is the existing contract of `BaseDatasourceClient.status()` and `testConnection()`'s catch path (see `packages/fs-datasource-engine/src/base-client.ts` lines ~280–315). The `authentication-failed` event is reserved for `authenticate()` and `withRefresh()` refresh-token failures and is NOT emitted by the status path. Bus subscribers therefore see the `auth-revoked` tag but not the `raw.kind` discriminator; subscribers needing the structured detail must read it from the thrown error returned to the direct caller (e.g., the IPC layer can serialize the full error in its response). This is sufficient for the scope-drift use case: the bus event tells the UI "go to AuthRevokedState"; tailored copy (deferred to `add-drive-oauth-browser-consent`) reads the structured `raw` from the IPC error response.
