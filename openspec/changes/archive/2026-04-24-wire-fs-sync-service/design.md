...
**What — progress bar visibility.** The Progress bar (10.3/10.4) renders if and only if `jobsByDatasource[id]` contains at least one `kind === "upload"` job whose status is `running | queued | waiting-network`. Bar value is computed from `uploadProgressByJob[activeUploadJobId]`: if `bytesTotal > 0` then `Math.round(bytesUploaded / bytesTotal * 100)`, else `0` (indeterminate state — bar renders at 0% until the first sized tick). Active job tiebreak rule (10.6): `createdAt desc`, then `jobId` lex desc. The bar unmounts naturally when the job is removed from `jobsByDatasource` (no explicit unmount logic needed; React reconciliation does it).

**Why `createdAt` rather than `startedAt`.** The wire `JobSummary` (`packages/ipc-contracts/src/sync-service/commands.ts`) carries `createdAt` + `updatedAt` only — there is no `startedAt` field. The `job-started` reducer case writes `payload.startedAt` into `updatedAt`, but every subsequent `job-progress` tick overwrites `updatedAt` to `Date.now()` for freshness, making it unstable for ordering. `createdAt` is set from `enqueuedAt` and never bumped, so it is the only stable per-job ordinal the renderer can read. The earlier draft of this section said `startedAt desc`; that was task-language carried over from the original task list and never reconciled with the wire shape.
...
## Appendix — Defects discovered during 10.9 smoke

Running the 10.9 smoke (real Google Drive credentials, live upload, interrupt and resume) surfaced six pre-existing defects that the unit-test suite did not catch. Five were fixed on this branch; the sixth is scoped out to a follow-up change. Recording them here so the fixes have provenance and so future spec archaeology can trace why they landed in this change rather than their apparent "owner" change.

All five fixes were invisible to the existing test suite because the test fakes bypass the real code paths.

1. **Dev ergonomics — `better-sqlite3` ABI conflict on Windows.** The service (Node, ABI 137) and desktop (Electron, ABI 145) share one compiled `.node` in the pnpm store. `pnpm dev` at the root cannot satisfy both on Windows (Unix allows `dlopen + unlink`; Windows holds the `.node` file-locked while loaded). Fix: introduced `bin/dev.sh` which builds, then flips to Electron ABI, then launches the service under `ELECTRON_RUN_AS_NODE=1` pointing at `node_modules/.pnpm/electron@*/.../electron.exe` — both processes run against Electron's bundled Node so a single rebuild satisfies both. README "Local development" rewritten to lead with `./bin/dev.sh start`. Stray-process sweep via PowerShell covers pre-`dev.sh` orphans.

2. **Google Drive strategy ESM/CJS require.** `packages/fs-datasource-engine/src/strategies/googledrive-client.ts` is ESM (`"type": "module"`), but `createDefaultDriveFactory` used a bare `require("googleapis")` to lazy-load the SDK. This threw `require is not defined` on first upload under real runtime. Unit tests missed it because they inject fake factories and never hit the default-factory branch. Fix: added `createRequire(import.meta.url)` shim at module top. The lazy-load intent is preserved; no API shape change.

3. **Upload executor misused `targetPath`.** `services/fs-sync/src/executors/upload.ts` passed `{ kind: "path", path: job.targetPath }` as the `parent` argument to `client.uploadFile`. `targetPath` is the full destination (e.g. `/ChatGPT.png`), not the parent folder, so the Drive strategy's `resolvePath` walked it as a folder path and threw `not-found: Path segment not found: /ChatGPT.png`. Fix: split `targetPath` via `posixPath.dirname` / `basename`, pass `parent = dirname`, `file.name = basename`. Test asserting the call shape was updated. The original test encoded the bug because the fake `uploadFile` didn't care how its args were structured.

4. **Engine progress events dropped on the floor.** The service instantiated `engineBus = createEngineEventBus()` in bootstrap and passed it to the client factory, but nothing subscribed to it. The engine's streaming `"uploading"` events (emitted on every chunk) never reached the service-level `bus`. The executor only emitted a single `job-progress` with `percent: 100` at completion. Fix: added `engineBus` to `UploadExecutorDeps`, subscribed before `uploadFile` (filtering on `event === "uploading" && datasourceId === job.datasourceId && payload.path === job.sourcePath`), translated each tick to a `job-progress`. Unsubscribed in `finally`. Bootstrap now passes `engineBus` through.

5. **Progress ticks carried no byte counts.** The engine bus emits only `progress` (0–100 percent), not bytes; the renderer's card derives the bar value from `bytesUploaded / bytesTotal` (store.tsx Decision 13) and ignores the wire `percent` field. Result: bar stayed at 0 even when ticks arrived. Fix: the upload executor pre-stats the source file (`statSync(job.sourcePath).size`) once, then synthesizes `bytesSent = Math.round(totalBytes * percent / 100)` and `totalBytes` on every emission. The terminal tick prefers the server-returned entry size, falling back to the pre-stat if absent.

6. **Datasource-card error text never healed after success.** The `ft5.db` `datasources.error_reason` column retained `"auth-revoked: ..."` strings from pre-`wire-fs-sync-service` code (the live-engine branch that used to write it was removed in section 9 but the SQLite rows persisted). No current production path writes or clears that column. Fix: added `jobDatasources: Map<jobId, datasourceId>` to the main-process sync event-bridge, seeded from `job-enqueued` + state-seed; on `job-completed`, the bridge calls `registry.setStatus(datasourceId, "connected")` to heal any lingering error row. `job-failed` / `job-cancelled` intentionally do NOT clear — they may represent legitimate error conditions. Renderer now also calls `refresh()` on `job-completed` so the card's summary re-fetches after the healing write. New test file: `apps/desktop/src/main/sync/event-bridge.error-healing.test.ts` (4 cases — completed clears, failed doesn't, cancelled doesn't, unknown-jobId no-ops).

7. **`sync-state-seed` dropped on fast reopen — progress bar didn't resume.** The main-process event-bridge broadcasts the seed via `webContents.send` the moment `doHandshake` completes. On a fresh desktop relaunch with a warm service (already running in its own process), the handshake returns in ~50 ms — typically BEFORE the renderer's React `useEffect` has run and registered the `ipcRenderer.on(SYNC_CHANNELS.event, ...)` listener. Electron has no listener-queue semantics for `webContents.send`, so the seed is silently dropped. Subsequent `job-progress` ticks arrive with jobIds the renderer's reducer doesn't know (no prior `job-enqueued` or seed for them), so the reducer's `findJobLocation` returns null and they are dropped too — card shows no progress bar even though the service keeps uploading and eventually writes the file successfully. Fix: pull the initial state via request-response on renderer mount (`window.api.sync.listJobs({filter: {statuses: ["running","queued","waiting-network"]}})`) and dispatch the jobs as a `sync/state-seed` action. Request-response has guaranteed delivery; the renderer asks when *it* is ready. The pushed `sync-state-seed` subscription stays for reconnect-path scenarios. Required widening the renderer's ambient `window.api.sync` type to include `listJobs` — previously narrowed to `onEvent` only per task 10.2's intentional-scope comment.

**Deferred — scoped to its own change.** The desktop's `files:*` IPC handlers (`list`, `stat`, `search`, `rename`, `remove`, `download`) all delegate to `apps/desktop/src/main/ipc/files/mock-fs.ts` — a UI-only fixture from the archived `ui-file-explorer` change. They were never wired to the service or engine. Consequence: the file explorer cannot browse actual datasource contents. This is out of scope for `wire-fs-sync-service` (which delivers upload end-to-end, not browsing). Follow-up change working name: `wire-file-explorer-to-service`. Engine primitives (`listFolder`, `getMetadata`, etc.) already exist on every strategy; the work is IPC plumbing plus pagination and error translation.

**Deferred — scoped to its own change.** The packaged build (electron-builder `--dir` / `--win` / etc.) was never end-to-end validated against this change's runtime requirements. The 13.4 attempt surfaced three compounding issues:

1. **Service binary not shipped.** `electron-builder.yml`'s `extraResources` only includes the renderer; `services/fs-sync/dist` is not copied. The supervisor's `servicePath` (`<resources>/fs-sync/main/index.js`) points at a file that doesn't exist in the packaged tree.

2. **Service dependencies don't resolve in packaged form.** Same root cause as defect #2's engine fix in dev mode — pnpm's `.pnpm` content store lives at the workspace root, not under any individual workspace's `node_modules/`. electron-builder's file walker only sees the desktop workspace tree, so transitive runtime requires from fs-sync (e.g. `better-sqlite3`, `drizzle-orm`) would fail at packaged runtime. Solving this needs one of: bundling fs-sync via esbuild into a single `index.js` (like we did for the engine in main.js — defect #2 partial fix already in `apps/desktop/electron.vite.config.ts`), `pnpm deploy --prod` to flatten the tree, or repo-wide `node-linker=hoisted`.

3. **Spawn target missing.** `node-binary-resolver.ts` resolves to `<resources>/node/win-x64/node.exe`. No standalone Node binary is bundled (the spike at `node-binary-resolver.spike.md` flagged this as "spike during task A2" but the implementation never landed). Result: `Error: spawn ...node.exe ENOENT`. Fix should pivot to `ELECTRON_RUN_AS_NODE=1` against `process.execPath` — same trick `bin/dev.sh` uses to avoid an ABI flip — which means modifying the supervisor's `spawn` to take an `env` option.

Partial fix already on this branch: `apps/desktop/electron.vite.config.ts` excludes `@ft5/fs-datasource-engine` from externalization so the engine + its `@aws-sdk/*` + `googleapis` + `@smithy/*` transitive chain is bundled into `dist/main/index.js`. `apps/desktop/electron-builder.yml` includes `node_modules/**/*` for the remaining direct deps (`better-sqlite3`, `electron-updater`, `drizzle-orm`). These resolve the *desktop-side* module-not-found errors that 13.4 surfaced; the *service-side* issues above remain.

Follow-up change working name: `wire-packaged-build-fs-sync-service`. Scope: bundle/deploy fs-sync, ship via `extraResources`, switch supervisor to `ELECTRON_RUN_AS_NODE=1`, packaged-runtime smoke. The dev-mode smoke (10.9) is unaffected and proves the wiring this change actually delivers; the packaged-build smoke is genuinely a separate concern that touches build infrastructure another change (the archived `setup-project`) properly owns.

Also noted but not filed: the OAuth add-datasource flow remains mocked (Decision 11 — `implement-datasource-onboarding` is the chartered follow-up). Manual credential seeding (GCP OAuth client + `get-gdrive-tokens.ps1` + hand-written `~/ft5/sync_app/dev/credentials.json`) is documented in the README "Google Drive datasource setup (dev only)" section so the smoke path is reproducible.
