...
**Known gap — credential persistence depends on `implement-datasource-onboarding`.** The desktop main process is wired with `noopCredentialStore` per the broader architecture's "Decision 1 — credential ownership lives on the service" (wire-fs-sync-service). After `completeWith(code)` returns a valid `AuthResult`, the engine's `decorateIntent` calls `credentialStore.put(...)` — but the no-op store discards it. As a result, the new datasource appears in the registry but has no credentials registered with the fs-sync service; clicking the card to explore fails with `Failed to load: no credentials registered for datasourceId=...`.
→ Resolution path: the follow-up change `implement-datasource-onboarding` implements the service-side `sync:authenticate-start` / `sync:authenticate-complete` handlers (today they ship as `tag: "not-implemented"` stubs). After that change, the desktop broker forwards the `AuthResult` to the service via `client.authenticateComplete(...)`, the service writes via its `ConfigFileCredentialStore`, and the datasource is fully usable. Until then, end-to-end smoke testing (Section 12 of this change's tasks.md) requires either (a) a manual `~/ft5/sync_app/dev/credentials.json` seed, or (b) the `FT5_DEV_CREDENTIALS=1` dev override path. This is a deliberate phased rollout — this change ships the consent UI/broker; the follow-up wires persistence.
