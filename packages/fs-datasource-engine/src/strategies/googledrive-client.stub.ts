// Phase 5 placeholder for the Google Drive strategy.
//
// The real `GoogleDriveClient` lands in Phase 8 (wires `googleapis`). Until
// then, this stub lets the Phase-5 factory plumbing be exercised end-to-end.
// Any `doX` call throws a normalized `provider-error` with
// `raw: "not-yet-implemented:<method>"`.
//
// When Phase 8 lands, DELETE this file and replace the registry entry in
// `../factory.ts` with the real `GoogleDriveClient` factory.
...
type GDriveType = "google-drive";

class GoogleDriveClientStub extends BaseDatasourceClient<GDriveType> {
  readonly type: GDriveType = "google-drive";
  ...
  protected override async doGetMetadataImpl(
    target: Target,
  ): Promise<FileMetadata<GDriveType>> {
    void target;
  ...
  protected override normalizeErrorImpl(
    raw: unknown,
  ): DatasourceError<GDriveType> {
    if (raw instanceof DatasourceError) {
      return raw as DatasourceError<GDriveType>;
    }
    return new DatasourceError<GDriveType>({
      tag: "provider-error",
  ...
  private notYetImplemented(method: string): DatasourceError<GDriveType> {
    return new DatasourceError<GDriveType>({
      tag: "provider-error",
      datasourceType: "google-drive",
      datasourceId: this.datasourceId,
      retryable: false,
      raw: `not-yet-implemented:${method}`,
      message: `GoogleDriveClient.${method} not implemented — Phase 8 will land this`,
    });
  }
}
...
export const createGoogleDriveClientStub: ProviderFactoryFn<"google-drive"> = (
  ...
  void credentials;
  return new GoogleDriveClientStub({ datasourceId, ctx });
};
