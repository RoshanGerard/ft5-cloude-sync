...
export class GoogleDriveClient extends BaseDatasourceClient<"google-drive"> {
  ...
  // returned FileEntry. Intermediate-segment ambiguity is silently
  ...
  // -------------------------------------------------------------------------
  // Build FileEntry / FileMetadata
  // -------------------------------------------------------------------------

  /**
   * Map a Drive `files.list` / `files.get` response to a `FileEntry`. The
  ...
  private buildFileEntry(
  ...
  ): FileEntry<"google-drive"> {
    const id = file.id ?? "";
    const name = file.name ?? "";
    const folder = isFolder(file);
    const kind: "file" | "folder" = folder ? "folder" : "file";
    const mimeFamily: MimeFamily = folder
      ? "folder"
    ...
    const entry: FileEntry<"google-drive"> = {
      path: opts.path,
      handle: id,
      name,
      kind,
  ...
  protected override async doListDirectoryImpl(
    target: Target,
  ): Promise<FileEntry<"google-drive">[]> {
    ...
    const entries: FileEntry<"google-drive">[] = [];
  ...
  protected override async doSearchImpl(
    ...
    const out: FileEntry<"google-drive">[] = [];
    for (const file of resp.files ?? []) {
      const name = file.name ?? "";
